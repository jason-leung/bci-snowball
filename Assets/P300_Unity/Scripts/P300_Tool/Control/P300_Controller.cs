...
Adapted from: Shaheed Murji "P300_Flashes.cs"
    ...
    Parameters here have been updated, and the master script has been broken out to better support modularity.
...
public class P300_Controller : MonoBehaviour
{
    ...
    public List<GameObject> object_list_p1 = new List<GameObject>();  //Previously 'cube_list'. List of objects that will be flashing, shared with the LSL inlet.
    public List<GameObject> object_list_p2 = new List<GameObject>();
    ...
    [SerializeField]
    public SingleFlash singleFlash;
    ...
    public void TurnOffObject(int i)
    {
        if (i >= 0 && i < object_list_p1.Count)
        {
            object_list_p1[i].GetComponent<Grayscale>().EnableGrayscale();
            object_list_p2[i].GetComponent<Grayscale>().EnableGrayscale();
        }
    }

    public void TurnOnObject(int i)
    {
        if (i >= 0 && i < object_list_p1.Count)
        {
            object_list_p1[i].GetComponent<Grayscale>().DisableGrayscale();
            object_list_p2[i].GetComponent<Grayscale>().DisableGrayscale();
        }
    }
